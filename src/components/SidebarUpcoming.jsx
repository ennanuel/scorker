        ...
        <ul className="match_lists">
            {
                matches.slice(0, 5).map( (match, i) => {
                    ...
                    return (
                        <li key={i} className="match_list full-border cursor-pointer flex-row ai-c jc-sb">{match.homeTeam.shortName} - {match.awayTeam.shortName} <span>{hrs < 10 ? '0' + hrs : hrs}:{mins < 10 ? '0' + mins : mins}{hrs >= 12 ? 'PM' : 'AM'}</span></li>
                    )
                })
            }
        </ul>
